...
exports.writeToFile = co.wrap(function * (languageCodeMap, solutionsDirPath) {
    // don't write empty question 
    if(!languageCodeMap['_question']) return;
    ...
    try {
        ...
        let questionMd = `# [${languageCodeMap._id}.${languageCodeMap._upperTitle}](https://leetcode.com/problems/${languageCodeMap._title}/)
        \n## Description
        \n${languageCodeMap['_question']}`
        promises.push(write(path.resolve(outputPath, 'question.md'), questionMd))
        yield promises
    } catch (err) {

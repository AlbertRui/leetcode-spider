...
let fetchACLists = co.wrap(function * () {
    ...
    // fetch DataBase
    let [, dataBase] = yield request.get('/api/problems/database/')
    try {
    ...
    leetcodeNumObj.total = body['num_total'] + dataBase['num_total']
    leetcodeNumObj.solved = body['num_solved'] + dataBase['num_solved']
    leetcodeNumObj.locked = 0
    return [...body['stat_status_pairs'],...dataBase['stat_status_pairs']]
})
...
let fetchWithGivenNumber = co.wrap(function * (acLists, numObj) {
    ...
    return acLists.filter(element => {
        ...
        // frontend_question_id is real id
        //! (element.stat['frontend_question_id'] in result)
        // if we fetched the problem before, we will not do it again
        return element.status === 'ac' && (element.stat['frontend_question_id'] in numObj)
    }).reverse()
})
...
let fetchSolutionsNotEverFetched = co.wrap(function * (acLists) {
    ...
    return acLists.filter(element => {
        ...
        let item = result[element.stat['frontend_question_id']]
        //! (element.stat['frontend_question_id'] in result)
        // if we fetched once we will not fetch this problem any more
        // 第一次爬取失败的题目，第二次再次爬取该题目
        return element.status === 'ac' && (!item || !item.language)
    }).reverse()
})
...
let parallelFetch = co.wrap(function * (acLists) {
    ...
    if (acLists && acLists.length > 0) {
        ...
        let acListPromises = acLists.map(acProblem => {
            ...
            languageCodeMap._upperTitle = acProblem.stat['question__title']
            languageCodeMap._id = acProblem.stat['frontend_question_id']
...
let fetchACSolutionOfProblem = co.wrap(function * (problemInfo, languageToFetch, page, languageCodeMap) {
    ...
    try {
        ...
        // debug.info(submissionsJson)
        // leetcode use graphGL
        let [, body] = yield request.post({
            ...
            body: {
                ...
                variables: {
                    offset: executionConfig.pageSize * page,
                    limit: executionConfig.pageSize ,
                    lastKey: null,
                    questionSlug: problemInfo.stat["question__title_slug"]
                },
                query:
        ...
        submissionsJson = body.data.submissionList.submissions;
    } catch (err) {
    ...
    // submissionsJson may be not array
    if(!Array.isArray(submissionsJson)) return languageCodeMap;
    // check the submissions list
    for(let e of submissionsJson) {
        ...
        if (~languageToFetch.indexOf(language)) {
            ...
            acSolutionPromise.push(fetchSolutionsOfUrl(codeUrl, 0).then(codeObj => {
                languageCodeMap[language] = languageCodeMap[language] || codeObj.code
                languageCodeMap._question = languageCodeMap._question || codeObj.question
            }).catch(err => {
    ...
    if (acSolutionPromise.length < 1) {
        if (submissionsJson.length === executionConfig.pageSize && languageToFetch.length > 0) {
...
let fetchSolutionsOfUrl = co.wrap(function * (url, times) {
    ...
    try {
        ...
        debug(e.stack)
        if (times < executionConfig.repeatTimes) {
    ...
    // get question description too
    let $ = cheerio.load(body);
    let questionText = $("meta[name=description]").attr("content");
    ...
    debug(url + 'code get!')
    return {code,question: questionText}
})
